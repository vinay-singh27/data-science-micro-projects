...
def extractor(img_path, model) :
    '''
    Function to convert the image into
    an array for representation
    '''
...
def feature_extractor(config_path, params_path) :
    ...
    model = VGGFace(model = model_name, include_top = include_tops,
                    pooling = poolings, input_shape =(224,224,3) )
...
if __name__ == '__main__' :
    ...
    try :
        ...
        feature_extractor(config_path= parsed_args.config, 
                                params_path= parsed_args.params )
        logging.info("state_02 completed >>>>>")
